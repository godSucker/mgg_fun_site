...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  // Check if this is a request to our webhook endpoint
  if (url.pathname === '/api/telegram-webhook') {
    if (context.request.method === 'POST') {
      try {
        // Verify this is coming from Telegram (optional for testing)
        // const token = context.request.headers.get('X-Telegram-Bot-Api-Secret-Token');
  ...
  // If not our webhook, continue with normal processing
  return next();
});