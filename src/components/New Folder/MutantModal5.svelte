...
<div class="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto overscroll-contain" on:click|self={close}
  ...
  <div class="mt-4 space-y-4" id="mutant-details">
...
<style>  .stat-icon { width: 18px; height: 18px; display:inline-block; }
  .row-icon { display: inline-flex; align-items: center; gap: .4rem; }

  /* Card is now a single face; keep containers neutral */
  .flip-scene{}
  .flip-card{ position: relative; }
  .flip-face{ position: static; transform: none; backface-visibility: visible; -webkit-backface-visibility: visible; }
</style>
