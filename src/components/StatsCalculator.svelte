<script lang="ts">
  ...
  // Import the mutant data for each star tier.  These files live at the
  // root of the repository and were generated in previous steps.  They
  // contain extra fields such as `atk1_base`, `atk2_base`, `atk1p_base`
   ...
   * normal mutants, 1 is bronze, 2 is silver, 3 is gold and 4 is
   * platinum.  When the user changes the star slider we select the
   * appropriate dataset.
   */
  const starDatasets = [normalData, bronzeData, silverData, goldData, platinumData];

  /**
   * Gene filter definitions.  The `letter` corresponds to the gene
   * prefix (always lowercase) used in the mutant definitions.  The
   * `icon` property points to a location in the static assets where
   * gene icons are stored.  A final entry of `all` resets the filter.
   */
  ...
  /**
   * We categorise the orbs into basic and special.  The simple rule
   * below examines the id and name to decide which bucket it belongs
   * to.  You can refine this if your orb naming convention changes.
   */
  ...
  let starLevel = 0; // 0 normal, 1 bronze, 2 silver, 3 gold, 4 platinum
  let levelValue = 1; // mutant level (1..100+)
  // Basic orb slots: by default three, but heroics use four.  We
  // initialise with null meaning no orb selected yet.
  let basicSlots: (any | null)[] = [null, null, null, null];
  // Special orb slot: null when empty
  let specialSlot: any | null = null;
  // Track which slot menu is currently open (index of slot or 'special')
  ...
  function handleDocumentClick(e: MouseEvent) {
    ...
    const clickedInsideMenu = path.some(el => (el as HTMLElement).classList && (el as HTMLElement).classList.contains('orb-menu'));
    const clickedSlotButton = path.some(el => (el as HTMLElement).classList && (el as HTMLElement).classList.contains('orb-slot-button'));
